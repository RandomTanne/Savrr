...
export default function YoutubeVideoComponent({video, id}) {
    ...
    return (
        <div ref={setNodeRef} style={style} className="p-3 rounded bg-primary">
            <Link href={video.url} target="_blank">
                <img className="rounded" src={url} width={300}/>
            </Link>
            <div>Test hehe</div>
            <Bars3Icon className="size-7 absolute bottom-1 right-1 bg-accent p-1 rounded-full" {...listeners} {...attributes}></Bars3Icon>
        </div>
    )
}